package com.david.todo.integration;
...
class TodoApplicationTests {
	...
	@Autowired
	public TodoApplicationTests(TaskController taskcontroller, MockMvc mockMvc) {
	...
	public void getOneTodoTest() throws Exception {
		this.mockMvc.perform(get("/tasks/1")).andDo(print()).andDo(print()).andExpect(status().isOk())
				.andExpect(content().string(containsString("\"id\":1,\"status\":")));
	}

	@Test
	public void getNonExistentTodoTest() throws Exception {
		this.mockMvc.perform(get("/tasks/100")).andDo(print()).andDo(print()).andExpect(status().isBadRequest())
	...
	public void getPendingTodoTest() throws Exception {
		...
		this.mockMvc.perform(get("/tasks/status/" + status)).andDo(print()).andDo(print()).andExpect(status().isOk())
	...
	public void getInvalidStatusTodoTest() throws Exception {
		...
		this.mockMvc.perform(get("/tasks/status/" + status)).andDo(print()).andDo(print())
